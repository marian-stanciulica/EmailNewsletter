...
async fn validate_credentials(credentials: Credentials, pool: &PgPool) -> Result<uuid::Uuid, PublishError> {
    ...
    if let Some((stored_user_id, stored_password_hash)) = get_store_credentials(&credentials.username, &pool)
        .await
...
async fn get_store_credentials(username: &str, pool: &PgPool) -> Result<Option<(uuid::Uuid, Secret<String>)>, anyhow::Error> {
    let row = sqlx::query!(
        r#"
